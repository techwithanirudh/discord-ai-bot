...
export function logTrigger(ctxId: string, trigger: { type: TriggerType; info: string | string[] | null }) {
  if (trigger.type) {
    logger.info(
...
export function logReply(ctxId: string, author: string, result: { success?: boolean; response?: string; error?: string }, reason?: string) {
