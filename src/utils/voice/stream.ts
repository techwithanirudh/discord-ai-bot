...
import type { VoiceReceiver, VoiceConnection, PlayerSubscription, AudioPlayer } from '@discordjs/voice';
...
export async function createListeningStream(
  ...
  ogg.on('end', async () => {
    ...
    console.log(`🎤 Captured ${audioBuffer.length} bytes of Ogg for ${user.username}`);
