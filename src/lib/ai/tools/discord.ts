...
export const discord = ({
  ...
  tool({
    ...
    parameters: z.object({
      target: z
        .enum(["message", "channel", "user", "guild", "guilds", "client", "messages"])
    ...
    execute: async ({ target, path, method, args, targetId }) => {
      ...
      logger.info({
        target, path, method, args, targetId, timestamp: Date.now()
      });
      ...
      logger.info(JSON.stringify(result), "Discord tool result");
